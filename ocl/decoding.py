...
class EntitySegDecoder(nn.Module):
    def __init__(
        self,
        # decoder: nn.Module,
        # num_classes: int,
    ...
    # n_patches = patches.shape[-1]
        ...
        # input = input.view(-1, n_channels, patch_size, patch_size) - happens in resizing when patch = True

        # dummies are required for loss computation which we do not need
        return PatchReconstructionOutput(
...
class EntityNetV2(DefaultPredictor):
    ...
    def __init__(self, config_file, opts, confidence_threshold, instance_mode=ColorMode.IMAGE):
        """
        Args:
            cfg (CfgNode):
